...
class PythonParser(LanguageParser):
    ...
    def parse(self, path: Path, repo_root: Path):
        ...
        try:
            ...
            # RAISE, DO NOT RETURN EMPTY. `return [], []` says "this file
            # contains nothing", which is indistinguishable from an empty
            # file - so a file with a syntax error was indexed as zero
            # entities and every surface downstream reported success.
            # verify-change said `changed: 0` and --gate exited 0 on a
            # change whose impact had never been computed: a green light
            # generated by not looking.
            #
            # The indexer already wraps parse() and records what it
            # catches; it just never had anything to catch here.
            raise SyntaxError(
                f"{path}: {e.msg} at line {e.lineno}") from e
...
PARSE_SCHEMA_VERSION = 13     # 13: comments carry `h`, a digest of the
                              ...
                              #     contains nothing' - the exact silence
                              #     the change removes.
                              # 11: RawEdge.literal_recv - the receiver
