...
# Manifest v1. This is the schema, and this reader is the only place that
# decides what a manifest means - the assembly side writes it, nothing else
# interprets it.
#
#   name            the map's own name, e.g. "httpx"
#   upstream_repo   URL of the repository that was indexed
#   upstream_sha    the exact commit indexed; drift is measured against it
#   memway_version  the memway that built it
#   license         upstream's license, carried with the map
#   built_at        UTC ISO-8601 build time
#
# LEGACY FALLBACK: the first three bundles published (itsdangerous, flask,
# httpx) shipped a manifest carrying BOTH v1 fields and the older
# repo/sha/version aliases, because the reader at the time only understood
# the aliases. Those releases are live and must keep installing, so the
# aliases stay readable - but they are read INTO the v1 names, so nothing
# downstream of this function has to know two schemas exist.
MANIFEST_V1 = ("name", "upstream_repo", "upstream_sha",
               "memway_version", "license", "built_at")
LEGACY_ALIASES = {
    "upstream_repo": ("repo", "source"),
    "upstream_sha": ("sha",),
}
...
def _describe(coord: Path) -> dict:
    ...
    Returns v1 field names regardless of which schema the bundle used.
    """
    info = {"entities": None, "manifest_schema": None}
    info.update({k: None for k in MANIFEST_V1})
    try:
    ...
    if not isinstance(man, dict):
        return info
    for k in MANIFEST_V1:
        if man.get(k):
            info[k] = man[k]
    for canon, aliases in LEGACY_ALIASES.items():
        if info[canon]:
            continue                      # v1 wins wherever it is present
        for a in aliases:
            if man.get(a):
                info[canon] = man[a]
                break
    # The label reports which shapes the bundle CARRIES, not which one won
    # the lookup. That is the question worth answering later: the alias
    # reader can retire once nothing published is legacy-only.
    has_v1 = any(man.get(k) for k in MANIFEST_V1)
    has_legacy = any(man.get(a) for al in LEGACY_ALIASES.values() for a in al)
    info["manifest_schema"] = ({(True, True): "v1+legacy", (True, False): "v1",
                                (False, True): "legacy"}
                               .get((has_v1, has_legacy)))
    return info
...
def pull(name: str, into: str = ".", source: str = DEFAULT_SOURCE,
    ...
    # The requested name/version win as the install identity, so preserve
    # what the bundle called itself rather than silently overwriting it -
    # a mismatch is worth being able to see.
    report["manifest_name"] = report.get("name")
