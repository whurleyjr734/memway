...
`main()` intercepts `--json` BEFORE dispatch and reads the next token as
a QUERY NAME, so the flag on `cmd_review` never ran - it answered
`no JSON query for 'review'`. The flag was dead from the moment it was
written, and it shipped. Same shape as the `--replay` flag that shipped
discoverable by nobody, and found the same way: by using the tool.

`review` is a registered `--json` query now - one door, the established
one - and the CLI error names the working form, because somebody who
learned the flag will type it again. The revision comes positionally or
as `--since=REV`.

### The falsification found dead code

The revision parser had a branch for `--since REV` that no sabotage could
break: the flag is skipped as a `--` token and the value after it is
caught by the positional case, so the branch never decided anything.
Removed. `--since=REV` keeps its own case - it starts with `--`, so
without it the revision silently stays at the default.

Three vacuous tests preceded one that bit, all the same mistake: the
assertion used `HEAD`, which is the DEFAULT, so it passed whether or not
the argument was parsed at all. The fixture uses the commit sha now -
a value no fallback can produce.

### From exercising the 0.60.0 tools

- `search "confidence"` surfaces the original 2026-08-16 resolution
  defect note, which is what an agent touching resolution needs and
  could not previously find.
- django markers **114 -> 45** under the new attribution: 61% was
  duplication, one genuine duplicate remains.
- `knowledge_at_risk` correctly returns 0 on a `short_of` edit - every
  note naming it is superseded, and superseded is history somebody
  already answered.
